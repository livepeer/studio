...
The easiest way to the stream playback URL is to login to the Livepeer.com Dashboard, navigate to the streams list page, [livepeer.com/app/user](/app/user), and click on a stream name. The playback URL is listed on the page.
...
Users can view the live stream with any HLS video player (for example [video.js](https://videojs.com/) or [JW Player](https://www.jwplayer.com/)).

## Stream delivery via CDN

Playback for all streams is delivered via a content delivery network (CDN). The transcoded video stream automatically switches between the transcoded renditions based on the viewers’ available bandwidth, device performance, and network conditions.
