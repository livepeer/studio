...
Your or your application users need to be able to push an RTMP stream. To broadcast a live stream, you’ll need a stream key.

Here are several ways to find your stream key:

- Login to the Livepeer.com Dashboard, navigate to the streams list page, [livepeer.com/app/user](/app/user), and click on a stream name. Then copy your secret stream key.
...
When configuring the software to push streams, use the secret `streamKey` and the Livepeer.com RTMP ingest URL, `rtmp://rtmp.livepeer.com/live`.
