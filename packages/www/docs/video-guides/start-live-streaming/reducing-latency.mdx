...
Also, If you created your stream via the Livepeer.com API, confirm your profile bitrate settings are [reasonable](https://developer.apple.com/documentation/http_live_streaming/hls_authoring_specification_for_apple_devices). 

Lastly, sometimes, the source stream is causing poor playback performance even though you are using the globally-optimized URLs above and good bitrate settings. The source stream is the stream sent from your broadcast software. Livepeer serves the source stream with the transcoded renditions via the playback URL.

One way to test if the source stream is causing problems is by inputting the playback URL in the Livepeer.com [Test Player](/app/test-player). The test player is only available to logged in users.
