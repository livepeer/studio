...
[livepeer.com/app/user](/app/user), and click on a stream name. Look at the
Status row to see if the stream is Active or Idle. If the status Idle, that
means that Livepeer.com is not ingesting any video with that stream’s
`streamKey`.
