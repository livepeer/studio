...
The easiest way to confirm a live stream is active is to login to the Livepeer.com Dashboard, navigate to the Streams page, [livepeer.com/dashboard/streams](https://livepeer.com/dashboard/streams), and click on a stream name. Look at the Status row to see if the stream is Active or Idle. If the status Idle, that means that Livepeer.com is not ingesting any video with that stream’s `streamKey`.
