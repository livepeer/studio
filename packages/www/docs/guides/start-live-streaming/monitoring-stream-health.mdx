...
description: Understanding stream performance on Livepeer Studio
metaTitle: Monitoring stream health - Livepeer Studio
metaDescription: Understanding stream performance on Livepeer Studio
---
...
- **_Transcoding_** — A `Healthy` status indicates that video transcoding is
  ...
  [Status page](https://status.livepeer.studio/) for platform-wide issues.
- **_Real-time —_** This indicates whether our system is transcoding the stream
  ...
  steps
  [in this article,](https://livepeer.studio/docs/guides/start-live-streaming/debug-live-stream-issues)
...
<Alert css={{ mt: "$5", mb: "$5" }} variant="loContrast">
  <Text size="3" variant="gray">
    ...
    it, let us know at{" "}
    <a href="mailto:hello@livepeer.studio?subject=New%20Stream%20Health%20API%20user&body=Please%20include%20details%20about%20your%20app.">
      <code>help@livepeer.studio</code>
    </a>
...
- Events (`/data/stream/{streamId}/events`): Provides direct access to the
  low-level events sent by Livepeer Studio video-processing services. It also
  allows subscribing via SSE to receive new events as they happen, and is useful
  for debugging. This API powers the log feed in the dashboard.

Now let's go into more detail about each of the mentioned API endpoints.

### Health Status

##### Request: `GET livepeer.studio/data/stream/{id}/health`
...
- All `timestamps` are representend in milliseconds since the Unix epoch, which
  is the default representation of timestamps across all the API.
...
##### Request: `GET livepeer.studio/data/stream/{id}/events`
