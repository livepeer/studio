...
## Stream Health API <Badge size="2" variant="violet" css={{ ml: "$2" }}>Beta</Badge>
...
<Alert css={{ mt: "$5", mb: "$5" }} variant="loContrast">
  <Text size="3" variant="gray">
    ...
    it, let us know at hello@livepeer.com so we can give you a heads up before
    any changes that may affect the functionality.
  </Text>
</Alert>
...
information. It consists of the following endpoints:
...
<Alert css={{ mt: "$5", mb: "$5" }} variant="loContrast">
  <Text size="3" variant="gray">
    Heads up! The Stream Health API uses <code>origin.livepeer.com</code>
    instead of <code>livepeer.com</code> as the hostname. We’d like to keep them
    consistent so we’re working on having everything on the
    <code>livepeer.com</code> domain soon.
  </Text>
</Alert>

Now let's get into more detail on each of the Stream Health API endpoints.
