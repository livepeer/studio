...
description: Back-up transcoding with Livepeer Studio
metaTitle: Redundant back-up transcoding - Livepeer Studio
metaDescription: Back-up transcoding with Livepeer Studio
---
...
- **Riskier during unforseen outages**: On the rare occasional that a Livepeer
  Studio ingest data center goes completely offline, all active live streams
  into that datacenter region will need to restart their streams using a
...
calling `https://livepeer.studio/api/ingest`. For example:
...
`https://livepeer.studio/api/ingest?first=false`. For example:
