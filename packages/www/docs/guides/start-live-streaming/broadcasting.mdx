...
description: Configure broadcast software to stream with Livepeer Studio
metaTitle: Configure broadcast software - Livepeer Studio
metaDescription: Configure broadcast software to stream with Livepeer Studio
---
...
- Login to the Livepeer Studio Dashboard, navigate to the streams list page,
  [livepeer.studio/dashboard/streams](https://livepeer.studio/dashboard/streams),
  and click on a stream name. Reveal and copy your secret stream key.
- When you create a `stream` object with a `POST` request to `/api/stream`, note
  the value of the `streamKey` in the response.
- Make a `GET` request to `/api/stream/[id]`, and note the value of the
