...
reference an existing `Multistream Targets` by `id` or create a new
`Multistream Target` inline with the `spec` field. You can also have both types
of targets on the same stream (reference and inline) as long as they are
...
referenced with an `id` or created inlined with a `spec`.
