...
that they are only allowed to call some very specific endpoints of the API. This
is another intermediate security measure, so that even if your API key is from
your web page, malicious actors won't be able access some of the most sensitive
APIs that expose PIIs or resources created by other users.

The logic here was that we only allow APIs for creating resources (e.g. streams
or assets) and for reading or manipulating those, as long as you already have
the ID for those.
...
Full access is even riskier. But you can also enable it.
