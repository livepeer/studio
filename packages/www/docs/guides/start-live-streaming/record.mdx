...
_Please note if you are using OBS, set your keyframe interval to 2 for Livepeer
Studio recording to work best._
...
Livepeer Studio Dashboard, navigate to the Streams page (
[livepeer.studio/dashboard/streams](https://livepeer.studio/dashboard/streams))
and click on a stream name. On the stream detail page, you can toggle the
recording setting or use the Actions button in the top right hand corner and
select Enable/Disable recording. All new sessions will adopt the updated
recording setting, but changing the settings will not change the recording
preference for any active sessions.
