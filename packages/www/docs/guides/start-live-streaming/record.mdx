...
axios.patch("https://livepeer.com/api/stream/{id}/record"),
  {
    headers: {
      Authorization: "Bearer {api_key}",
      "Content-Type": "application/json",
...
axios.get("https://livepeer.com/api/stream/{id}", {
  headers: {
    'Authorization': 'Bearer {api_key}',
    'Content-Type': 'application/json'
  }
...
axios
  .get("http://livepeer.com/api/session/{id}", {
    headers: {
      Authorization: "Bearer {api_key}",
      "Content-Type": "application/json",
    },
  ...
  .then((res) => {
    console.log(JSON.stringify(res.data)).catch((error) => {
      console.log(error);
    });
  });
```
