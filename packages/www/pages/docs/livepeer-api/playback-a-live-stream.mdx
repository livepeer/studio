...
Once the broadcast session is started, transcoding will automatically happen.  Users can view the live stream with any HLS video player by using the playback URL: `{playback_base_url}/{playbackId}/index.m3u8`.  (for example, with [video.js](https://videojs.github.io/videojs-contrib-hls/)) 

You can use fetch and examine the playlist by using the playback URL, which should give you the full HLS playlist that includes all 4 different renditions.  For example:
