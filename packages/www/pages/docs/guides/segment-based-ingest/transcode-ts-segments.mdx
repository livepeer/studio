...
[create a stream with defined rendition profiles](/docs/segment-based-ingest/create-a-stream-and-define-renditions)
and
[gather a list of broadcasters](/docs/segment-based-ingest/get-a-list-of-broadcasters).
It may also be helpful to review how to
[authenticate](/docs/segment-based-ingest/authentication).

`POST` your .ts segment. to
...
If you encounter a problem uploading a segment, try making a new request for a
list of broadcasters and `POST` to a different broadcaster address. You can
still expect the stream to be at the same URL.

Mixed within the data about your transcoded video, the content of the API
