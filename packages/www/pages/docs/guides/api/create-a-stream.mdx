...
This creates a new *parent* `stream` object. The `id` in the POST request response will be the `parentId` for all future *session* `stream` objects for this stream. This guide explains the [relationship between parent-config and child-session `stream` objects](/docs/guides/api/parent-streams-and-sessions)
