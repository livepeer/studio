...
The `/stream` endpoint provides a list of all `stream` objects, including parent-config and child-session `stream` objects. The following filter are helpful for optimizing your platform with the Livepeer.com API.

Some of the examples show decoded JSON objects. But, remember that anything passed as a parameter in a GET request URL must be URL encoded.
...
### Return a list all sessions with `sessionsonly=1`
