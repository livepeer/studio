...
To verify a stream is running, you can fetch the stream status by sending a `GET` request to `https://livepeer.com/api/stream/` with the stream object `id` appended. You received the `id` in the response when you [created a stream object](/docs/guides/api/create-a-stream).
...
Next, learn [how to playback a live stream with the Livepeer.com API](/docs/guides/api/playback-a-live-stream).
