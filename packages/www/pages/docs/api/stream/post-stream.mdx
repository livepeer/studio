...
# `POST /stream` to create a `stream` object

`POST /stream` creates a _parent_ stream object. The _parent_ configuration is
inherited by all future child-session `stream` objects.
