...
configuration. The `recordingStatus` parameter tells you when the recording is
ready for playback a few minutes after the stream ends.

This recording feature provides Livepeer.com users with a link to an adaptive
bitrate streaming (ABR) recording of their completed livestream.
...
To turn recording on and off, update the `record` parameter on the `stream`
object. The `record` parameter is a boolean with a `false` default value.
...
> Understanding this core concept make using the recording feautre simple.
...
The absence of a `record` parameter on a `stream` object is equivalent to
`record: false`.

## Notable JSON parameters for a `stream` session with `record:true`

_A list of JSON parameters for the `stream` object is at
[https://livepeer.com/docs/api/stream](https://livepeer.com/docs/api/stream)._

- `id`: This string is the unique identifier for the asset, in this case a
...
- `createdAt`: This number is the timestamp when the livestream session began
...
- `sourceSegmentsDuration`: This number is the duration in seconds of asset
  ...
  the length of the livestream session and recording.
- `recordingStatus`: This boolean appears only when `record: true`, and it is
  either `ready` when the recorded livestream is available for playback or
...
Turns recording off. All new sessions will not be recorded. In progress sessions
...
If the `record` parameter is not returned in the JSON response, recording is
**not** turned on. An absent `record` parameter is equivalent to
`"record": false`.
