...
description: Tips for reducing latency with Livepeer Studio
metaTitle: Tips for reducing latency - Livepeer Studio
metaDescription: Tips for reducing latency with Livepeer Studio
---

# Tips for reducing latency

Livepeer Studio routes your live streams to the nearest available datacenter to
...
Livepeer Studio serves the source stream with the transcoded renditions via the
playback URL.

One way to test if the source stream is causing problems is by inputting the
playback URL in the Livepeer Studio
[Test Player](https://livepeer.com/app/test-player). The Test Player is only
...
by the Livepeer Studio CDN. This player is configured to only playback the
highest resolution available. Except for rare occasions, the highest resolution
is the non-transcoded source stream.
