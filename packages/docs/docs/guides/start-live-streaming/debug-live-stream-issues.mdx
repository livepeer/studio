...
We will use the
[Livepeer Studio test player](https://livepeer.com/app/test-player) to debug.
You must be logged into the Livepeer Studio dashboard.

#### Prerequisite: Locate your playback URL.

Read this [guide](/guides/start-live-streaming/playback) to find your live
stream playback URL.

## Step 1

Load your playback URL in the
[Livepeer Studio test player](https://livepeer.com/app/test-player). If you get
a "Stream not found" error, confirm that your URL is correct. It should look
something like this: `https://livepeercdn.com/hls/123456abcdef7890/index.m3u8`
...
Does anything playback in the
[Livepeer Studio test players](https://livepeer.com/app/test-player)?
...
- **No:** No video is being ingested. Confirm your or your streamer’s broadcast
  ...
  [this blog post](https://livepeer.com/blog/first-livepeer-stream-in-five-minutes).
...
- **Yes:** You are not transcoding your source stream, and only the source
  profile is available for playback (no ABR) via the Livepeer Studio CDN. There
  may be something wrong with your source stream. If you can, instruct the
  streamer to set the keyframe interval to 2 in their broadcasting software;
  this can help. In general, make sure your broadcasting workflow is stable. The
  RTMP stream hitting Livepeer's ingest server has problems. Try ensuring a
  stronger internet connection at the source via a wired ethernet connection or
  by broadcasting on a more reliable network.
  <br />
...
stream sent from your broadcast software. Livepeer Studio serves the source
stream with the transcoded renditions via the playback URL.
...
- **Yes:** If you created your stream via the Livepeer Studio API, confirm your
  ...
  <a href="mailto:hello@livepeer.com?subject=Unknown%20live%20stream%20issue&body=Please%20include%20details.">
    help@livepeer.com
  </a>
  .
