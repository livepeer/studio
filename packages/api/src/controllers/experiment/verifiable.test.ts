...
beforeAll(async () => {
  ...
  res = await client.post(`/experiment/verifiable/audience`, {
    allowAll: true,
  });
  expect(res.status).toBe(204);

  client = new TestClient({ server });
  res = await client.post("/experiment/-/verifiable", REQUEST);
  expect(res.status).toBe(201);
  id = (await res.json()).id;

  res = await client.post("/experiment/-/verifiable", {
    primaryType: "Video",
    domain: {
      name: "Verifiable Video",
      version: "1",
...
describe("Verifiable Video API", () => {
  ...
  describe("POST /", () => {
    it("should return an error for invalid primaryType", async () => {
      ...
      const res = await client.post("/experiment/-/verifiable", request);
  ...
  describe("GET /:id", () => {
    it("should return the verifiable metadata with the given id", async () => {
      const res = await client.get(`/experiment/-/verifiable/${id}`);
      expect(res.status).toEqual(200);
    });

    it("should return error for non-existing verifiable metadata", async () => {
      const res = await client.get(`/experiment/-/verifiable/12345`);
      expect(res.status).toEqual(404);
    });
  });

  describe("GET /", () => {
    it("should return a list of verifiable metadata entries", async () => {
      const res = await client.get(`/experiment/-/verifiable`);
    ...
    it("should return a list of verifiable metadata entries from the given creator", async () => {
      const res = await client.get(
        `/experiment/-/verifiable?creator=${CREATOR_PUBLIC_KEY}`
      );
