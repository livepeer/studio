...
import { Box, Flex, Grid, TextField, Text } from "../";
...
function EditableScale({
  ...
  const isDarkTheme = document.body.classList.contains(
    darkThemeClassName.toString()
  );

  // Current curves
  const [lightThemeCurve, setLightThemeCurve] = React.useState(
    lightThemeConfig.defaultCurve
  );
  const [darkThemeCurve, setDarkThemeCurve] = React.useState(
    darkThemeConfig.defaultCurve
  );
  ...
  const showAlphaValues =
    (document.querySelector("[data-alpha-values]") as HTMLInputElement | null)
      ?.checked ?? false;

  // Refs to current values
  const curveRef = React.useRef<Curve>(
    isDarkTheme ? darkThemeCurve : lightThemeCurve
  );
  ...
  const onCurveChange = React.useCallback(
    (newCurve: Curve) => {
      ...
      const chromaCurve: Curve = [
        x1,
        y1 * boost1,
        Math.min(1, x2 * boost2),
        y2,
      ];
      ...
      if (!newColors.find((color) => color.name === `${name}10`)) {
        ...
        const baseColor =
          newColors.find((color) => color.name === `${name}${baseStep}`)
            ?.value ??
          computedStyles.getPropertyValue(`--colors-${name}${baseStep}`);

        const mixColor =
          newColors.find((color) => color.name === `${name}${mixStep}`)
            ?.value ??
  ...
  React.useEffect(() => {
    const getValue = (step: string) => {
      const cssVariableValue = computedStyles.getPropertyValue(
        `--colors-${name}${step}`
      );
    ...
    setFailsContrast(
      newContrasts.slice(0, -1).some((ratio) => ratio < 4.5) ||
        newContrasts[3] < 3
    );
        ...
        <Text
          ...
          onClick={(event) => {
            if (event.altKey) {
              document
                .querySelectorAll(
                  `[data-panel-toggle][data-panel-collapsed="${collapsed}"]`
                )
        ...
        {showResetButton && (
          <Box
            ...
            onClick={(event) => {
              ...
              onCurveChange(
                (isDarkTheme ? darkThemeConfig : lightThemeConfig).defaultCurve
              );
            }}
            css={{ p: "$1", flex: "none" }}>
            <ResetIcon />
        ...
        <Box
          ...
          onClick={(event) => {
            ...
            if (event.altKey) {
              document
                .querySelectorAll(
                  `[data-code-button][data-show-code="${showCode}"]`
                )
                .forEach((element) => (element as HTMLElement).click());
              return;
            }
            ...
            if (event.metaKey) {
              ...
              document
                .querySelectorAll(`[data-color-code]`)
                .forEach((element) => {
                  const codeToCopy = Array.from(
                    (element as HTMLElement).childNodes
                  )
          ...
          {showCode ? (
            <CodeIcon data-main-icon />
          ) : (
            <TextAlignJustifyIcon data-main-icon />
          )}
        ...
        <Box
          ...
          onClick={(event) => {
            if (event.altKey) {
              document
                .querySelectorAll(
                  `[data-active-toggle][data-active="${active}"]`
                )
                .forEach((element) => (element as HTMLElement).click());
              return;
            }
        ...
        <Box>
          <Grid
            ...
            {steps.map((step, index) => {
              const solidColorValue = computedStyles.getPropertyValue(
                `--colors-${name}${step}`
              );
              const backdropColor = isDarkTheme
                ...
                : "#ffffff";
              const alphaColorValue = getAlphaColor(
                solidColorValue,
                backdropColor
              );
              const valueToShow = showAlphaValues
                ? alphaColorValue
                : solidColorValue;
          ...
          {!showCode && (
            <Grid css={{ mx: "$2", gridAutoRows: "25px" }}>
              <RatioBox
                css={{ bc: `$${name}1`, color: `$${name}11` }}
                ratio={contrasts[0]}
              />
              <RatioBox
                css={{ bc: `$${name}2`, color: `$${name}11` }}
                ratio={contrasts[1]}
              />
              <RatioBox
                css={{ bc: `$${name}3`, color: `$${name}11` }}
                ratio={contrasts[2]}
              />
...
function Editor({ curve, onCurveChange }: EditorProps) {
  ...
  const inputValueIsValid =
    inputValueCurve.length === 4 &&
    inputValueCurve.every((number) => !isNaN(number));

  // Update input when value changes externally
  React.useEffect(() => {
    // Update input when curve editor updates
    const [x1, y1, x2, y2] = curve;
    const [X1, Y1, X2, Y2] = inputRef
      .current!.value.split(", ")
      .map(parseFloat);
      ...
      <Box css={{ px: "$2", mb: "$3" }}>
        <TextField
          ...
          onChange={(event) => {
            ...
            if (
              newCurve.length === 4 &&
              newCurve.every((number) => !isNaN(number))
            ) {
...
function prepareColorStringForChroma(color: string) {
  return /hsl/.test(color) && !/,/.test(color)
    ? color.replace(/\s+/g, ", ")
    : color;
}
...
function generateColors({
  ...
  for (const index in lumArray) {
    ...
    lumArrayAdjusted.push(
      distribute(step, [0, 1], [lumEnd * 0.01, lumStart * 0.01])
    );
  }
  ...
  for (const key in lumArray) {
    ...
    const color = chroma(
      chroma.lch(params.luminosity, params.chroma, params.hue)
    );
...
function getAlphaColor(targetColor: string, backdropColor: string) {
  ...
  const desiredRGB =
    targetR + targetG + targetB < backdropB + backdropG + backdropB ? 0 : 255;
