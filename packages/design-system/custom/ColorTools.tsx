...
import {
  darkTheme as darkThemeClassName,
  theme as lightThemeClassName,
        ...
        <Box
          ...
          onClick={(event) => {
            ...
            if (event.altKey) {
              document
                .querySelectorAll(
                  `[data-show-code-toggle][data-show-code="${showCode}"]`
                )
                .forEach((element) => (element as HTMLElement).click());
              return;
            }
        ...
        <Box>
          <Grid
            ...
            {Array.from(Array(11)).map((_, index) => (
              <Text
                ...
                {name}
                {index}00: '
                {computedStyles.getPropertyValue(`--colors-${name}${index}00`)}
                ',
              </Text>
            ))}
          </Grid>
          ...
          {!showCode && (
            <Grid css={{ mx: "$2", gridAutoRows: "25px" }}>
              <RatioBox
                css={{ bc: `$${name}000`, color: `$${name}900` }}
                ratio={contrasts[0]}
              />
              <RatioBox
                css={{ bc: `$${name}100`, color: `$${name}900` }}
                ratio={contrasts[1]}
              />
              <RatioBox
                css={{ bc: `$${name}200`, color: `$${name}900` }}
                ratio={contrasts[2]}
              />
...
function generateColors({
  ...
  for (const key in lumArray) {
    ...
    const colorObj: Color = {
      ...
      value: `hsl(${Math.round(h)} ${(s * 100).toFixed(1)}% ${(l * 100).toFixed(
        1
      )}%)`,
    };
